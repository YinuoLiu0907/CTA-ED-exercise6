...
## Exercise 1  Inspect and visualize the nearest neighbour synonyms of other relevant words in the tweets corpus

Setup
```{r, message=F}
library(tidyverse)   # loads dplyr, ggplot2, and others
library(stringr)     # to handle text elements
library(tidytext)    # useful functions for manipulating text
library(ggthemes)    # plot themes
library(text2vec)    # word embedding implementation
library(widyr)       # pairwise_count()
library(RSpectra)    # svd
library(umap)        # dimensionality reduction
library(tibble)      # rownames_to_column()
```

Read the tweets corpus
```{r}
twts_sample <- readRDS(
  gzcon(
    url("https://github.com/cjbarrie/CTA-ED/blob/main/data/wordembed/twts_corpus_sample.rds?raw=true")
  )
)
```
...
Check if the words are in the embedding
```{r}
c("climate", "education", "health", "housing") %in% rownames(glove_embedding)
```

Check the most related words
```{r}
## 查看和 “climate” 最相关的词
normalized_prob %>% 
    filter(word1 == "climate") %>%
    arrange(-p_together)
...
Remove missing values and run SVD
```{r}
pmi_matrix@x[is.na(pmi_matrix@x)] <- 0
...
Calculate the synonyms
```{r}
climate_synonyms <- nearest_words(word_vectors, "climate")
housing_synonyms <- nearest_words(word_vectors, "housing")
```

Check the results
```{r}
head(climate_synonyms, n = 10)
head(housing_synonyms, n = 10)
```
...
climate_synonyms %>%
  ...
  theme_tufte(base_family = "Helvetica") +
  labs(
    title = "Nearest neighbour words for 'climate' and 'housing'",
    x = NULL,
    y = "Similarity"
  )
```

The results show that both “climate” and “housing” are surrounded by meaningful and coherent terms, but they reflect different types of policy discourse.
For “climate”, the nearest words include terms like “change”, “tackle”, “crisis”, “global”, and “environment”. These words suggest that climate is mainly discussed in the context of urgency, global challenges, and collective action. There is a strong focus on problems and responses, such as tackling climate change or addressing a crisis.
In contrast, “housing” is associated with words like “affordable”, “homes”, “build”, “council”, and “infrastructure”. This indicates a more practical and policy-oriented discussion. The focus here is on provision, development, and local governance rather than global issues.
In conclusion, the comparison shows that the embedding captures different semantic fields: climate discourse is more abstract and global, while housing discourse is more concrete and policy-driven.


## Exercise 2 Identify another region of interest in the GloVe-trained model and visualize

Load pre-trained local GloVe embedding
```{r}
url <- "https://github.com/cjbarrie/CTA-ED/blob/main/data/wordembed/local_glove.rds?raw=true"
glove_embedding <- readRDS(url(url, method = "libcurl"))
```

Reduce GloVe embedding to 2D using UMAP
```{r}
glove_umap <- umap(
  glove_embedding,
  n_components = 2,
...
Put UMAP results in a dataframe for ggplot
```{r}
df_glove_umap <- as.data.frame(glove_umap[["layout"]])
...
```{r}
# choose seed word
word <- glove_embedding["climate", , drop = FALSE]
...
select <- data.frame(
  word = rownames(as.data.frame(head(sort(cos_sim[, 1], decreasing = TRUE), 25)))
)

# join with UMAP coordinates
selected_words <- df_glove_umap %>%
  inner_join(select, by = "word")

# visualize
ggplot(selected_words, aes(x = UMAP1, y = UMAP2)) +
  geom_point(show.legend = FALSE) +
  geom_text(
    aes(UMAP1, UMAP2, label = word),
    show.legend = FALSE,
    size = 2.5,
    vjust = -1.5,
    hjust = 0
  ) +
  labs(title = "GloVe word embedding of words related to 'climate'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```

The visualization of words related to “climate” shows a clear semantic cluster in the embedding space.
Words such as “climate”, “global”, “environment”, “crisis”, and “emergency” appear close together, which suggests that they are often used in similar contexts. This reflects a shared narrative around climate issues, especially the framing of climate change as a global emergency.
At the same time, words like “tackle”, “action”, and “government” are also nearby, indicating a focus on responses and policy actions. This suggests that discussions of climate are not only about the problem itself, but also about what should be done.
Overall, the cluster demonstrates that the GloVe model successfully groups together words that are semantically and contextually related in political discussions about climate.

Housing
```{r}
# choose seed word
word <- glove_embedding["housing", , drop = FALSE]
...
ggplot(selected_words, aes(x = UMAP1, y = UMAP2)) +
  ...
  labs(title = "GloVe word embedding of words related to 'housing'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```

The housing-related words also form a clear cluster. Terms such as “affordable”, “homes”, and “build” suggest a focus on practical policy issues and service provision. Compared to climate, the housing cluster appears more concrete and locally oriented.
